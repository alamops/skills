...
Delivery runs accept **modifier flags**, combinable with each other and the task text: `/implement --auto <task>` runs unattended for remote/CI execution, `/implement --no-e2e <task>` skips the end-to-end test layer (unit/integration coverage still runs), `/implement --no-spikes <task>` skips Phase 1's validation spikes (unsettled load-bearing assumptions are surfaced as open questions instead of experimentally verified), and `/implement --no-follow-ups <task>` removes the owner's option to defer remainder to a later ticket (any of `--no-follow-ups`, `--no-follow-up`, `--no-followups`, `--no-followup`, `--nofollowups`, `--nofollowup` selects it).

**Completeness is the default; `--no-follow-ups` is the stricter contract.** The usual way a delivery ends up *almost* right is a remainder: three of seven call sites still on the old path, the empty state left as a `TODO`, the enum grown in the model but not the validator, a migration that adds the column and never backfills the rows. Every run therefore carries a **completeness ledger** in the plan — each candidate follow-up marked *in this run* (with the owning task), *out of scope* (with why it's a different ticket), or *owner-deferred* (with who decided it at the grill or approval gate). Agents never defer on their own: implementation briefs ban `TODO`/stub placeholders and route out-of-bounds remainder back to the orchestrator, and the code review treats completeness findings as correctness, i.e. must-fix. What the owner may still do by default is consciously push a piece to a later ticket. `--no-follow-ups` removes that option too — the ledger has to close with nothing owner-deferred — which is also what makes it meaningful under `--auto`, where there's no owner to defer in the first place. Neither the default nor the flag is a scope expansion: adjacent features, drive-by refactors, `--no-e2e`/`--no-spikes` overrides and one-way decisions under `--auto` stay out, and a sweep that dwarfs the original ask goes to the owner at the approval gate rather than being absorbed silently.
